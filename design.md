# ScentHub — Design Brief for Claude
...
The experience should help shoppers discover fragrances, compare relevant details, choose a bottle size, and understand the price before adding an item to their bag. English and Arabic, light and dark themes, and small screens are first-class requirements.
...
Aim for a quiet fragrance boutique: warm backgrounds, precise typography, generous but purposeful space, and a clear focus on the bottle and product information. Use editorial composition without hiding the shopping task.
...
Create reference frames at 390px mobile, 768px tablet, and 1440px desktop. Also verify reflow at 320px and at browser zoom. These are review sizes, not device-specific layouts.
...
Build a small component library and reuse it across all screens. Each component needs size/spacing rules, semantic color mapping, content limits, theme behavior, RTL behavior, and interaction annotations.
...
- Preserve locale routing and the existing theme provider. Verify system-theme behavior and avoid theme-control hydration flashes during implementation.
...
1. A short creative rationale tied to ScentHub's fragrance catalog, warm palette, and bilingual audience.
2. A foundations sheet with exact semantic variables in light/dark modes, typography roles in both scripts, spacing, grids, control dimensions, and image treatments.
3. Reusable components covering the state matrix above, including keyboard focus and error recovery.
4. High-fidelity home, collection, product detail, and proposed bag screens at mobile and desktop sizes, plus a tablet layout demonstrating the intermediate behavior.
5. English/light reference screens and Arabic/dark versions of each primary screen. Include an Arabic/light collection and an English/dark product detail to check that theme and direction work independently.
6. A connected prototype covering browse → filter/search → product → change size → add to bag → change quantity/remove, plus language/theme switching. Mark missing backend behaviors explicitly.
7. Annotations for responsive changes, RTL behavior, price semantics, overflow, focus order, loading, empty, error, and unavailable states.
